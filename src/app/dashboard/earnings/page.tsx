...
export default function EarningsPage() {
    ...
    return (
        <div style={{ maxWidth: 1200, paddingBottom: 100, margin: '0 auto', fontFamily: 'var(--font-inter), sans-serif' }}>


            {/* --- HEADER SECTION --- */}
            <div style={{ background: '#fdfdfd', padding: '40px 0', borderBottom: '1px solid #eaeaea', marginBottom: 40, borderRadius: '0 0 24px 24px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', padding: '0 20px' }}>
                    {/* Level Badge */}
                    <div>
                        <h1 style={{ fontSize: '3rem', fontWeight: 800, margin: 0, lineHeight: 1, letterSpacing: '-0.03em', color: '#111' }}>Tutkun</h1>
                        <div style={{ marginTop: 12, color: '#666', fontSize: '1rem' }}>
                            <span style={{ fontWeight: 700, color: 'black' }}>Elçi seviyesine</span> 14 puan kaldı
                        </div>
                    </div>

                    {/* Stats Grid */}
                    <div style={{ display: 'flex', gap: 40, textAlign: 'right' }}>
                        <div style={{ background: 'white', padding: '20px 30px', borderRadius: 16, border: '1px solid #f0f0f0', boxShadow: '0 4px 20px rgba(0,0,0,0.02)' }}>
                            <div style={{ fontSize: '0.8rem', color: '#666', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8, fontWeight: 600 }}>Gelecek Ödeme</div>
                            <div style={{ fontSize: '2.2rem', fontWeight: 700, color: '#111', letterSpacing: '-0.02em' }}>-</div>
                        </div>
                        <div style={{ background: 'white', padding: '20px 30px', borderRadius: 16, border: '1px solid #f0f0f0', boxShadow: '0 4px 20px rgba(0,0,0,0.02)' }}>
                            <div style={{ fontSize: '0.8rem', color: '#666', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8, fontWeight: 600 }}>Bekleyen</div>
                            <div style={{ fontSize: '2.2rem', fontWeight: 700, color: '#f59e0b', letterSpacing: '-0.02em' }}>{formatCurrency(summary?.pendingBalance || 0)}</div>
                        </div>
                        <div style={{ background: 'white', padding: '20px 30px', borderRadius: 16, border: '1px solid #f0f0f0', boxShadow: '0 4px 20px rgba(0,0,0,0.02)' }}>
                            <div style={{ fontSize: '0.8rem', color: '#666', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8, fontWeight: 600 }}>Son Ödeme</div>
                            <div style={{ fontSize: '2.2rem', fontWeight: 700, color: '#10b981', letterSpacing: '-0.02em' }}>{formatCurrency(summary?.lastPayoutAmount || 0)}</div>
                        </div>
                        <div style={{ background: 'white', padding: '20px 30px', borderRadius: 16, border: '1px solid #f0f0f0', boxShadow: '0 4px 20px rgba(0,0,0,0.02)' }}>
                            <div style={{ fontSize: '0.8rem', color: '#666', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8, fontWeight: 600 }}>Toplam Kazanç</div>
                            <div style={{ fontSize: '2.2rem', fontWeight: 800, color: '#111', letterSpacing: '-0.02em' }}>{formatCurrency(summary?.totalEarnings || 0)}</div>
                        </div>
                    </div>
                </div>

                <div style={{ marginTop: 20, textAlign: 'right', padding: '0 20px' }}>
                    <a href="#" style={{ fontSize: '0.9rem', color: '#666', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 6, fontWeight: 500 }} className="hover:text-black transition-colors">
                        Ödeme Süreçleri Hakkında <HelpCircle size={16} />
                    </a>
                </div>
            </div>
            ...
            <div style={{ padding: '0 20px' }}>
                ...
                <Section title="Kazanç Grafiği">
                    <div style={{ background: 'white', border: '1px solid #eaeaea', borderRadius: 24, padding: '40px', height: 450, boxShadow: '0 10px 40px rgba(0,0,0,0.03)' }}>
                        <ResponsiveContainer width="100%" height="100%">
                            ...
                            >
                                ...
                                />
                                <Tooltip
                                    contentStyle={{ borderRadius: 12, border: 'none', boxShadow: '0 10px 30px rgba(0,0,0,0.1)', padding: '15px 20px' }}
                                    formatter={(value: number | undefined) => [<span style={{ fontWeight: 800, fontSize: '1.2rem', color: '#111' }}>{formatCurrency(value || 0)}</span>, '']}
                                    labelStyle={{ fontWeight: 600, color: '#666', marginBottom: 8, textTransform: 'uppercase', fontSize: '0.8rem', letterSpacing: 1 }}
                                />
                                <Area
                                    type="monotone"
                                    dataKey="total"
                                    ...
                                    fillOpacity={1}
                                    fill="url(#colorTotal)"
                                    activeDot={{ r: 8, strokeWidth: 4, stroke: 'white', fill: '#111' }}
                                />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
